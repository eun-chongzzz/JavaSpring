...
public interface BoardMapper {
	...
	// 파라미터는 필요없습니다. 글 개수 => 정수 값을 조회하기 때문에 int 리턴입니다.
	public int countPageNum(SearchCriteria cri);
	
	public void insert(BoardVO vo);
	
	// select구문은 글 번호를 입력받아서 해당 글 "하나"에 대한 정보만 얻어돌 예정이므로
	// 리턴자료형은 글 하나를 담당할 수 있는 BoardVO로 해야함
	public BoardVO select(long bno);
	
	// 글삭제는 DELETE 구문으로 하는데, 비SELECT구문이므로 리턴자료를 void로 적습니다.
	...
	// vo안쓰고 데이터 전달하기
	// 2개 이상의 파라미터를 따로따로 전달할때는 각 파라미터 왼쪽에 @Param("보낼명칭")을 붙여줍니다.
